...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# ⚡ OPTIMASI: Batch delete + bulk insert (lebih cepat 10-100x)
...
# Set faster mode for bulk operations
cursor_lengkap.execute("PRAGMA synchronous = OFF")
cursor_lengkap.execute("PRAGMA journal_mode = WAL")
...
# Get unique combinations of tanggal, jam, station_wmo_id for batch delete
...
# Batch delete: delete by unique keys (much faster than row-by-row)
unique_keys = df_to_insert_lengkap[["tanggal", "jam", "station_wmo_id"]].drop_duplicates().values.tolist()
if unique_keys:
    delete_placeholders = "DELETE FROM " + table_name_lengkap + " WHERE tanggal = ? AND jam = ? AND station_wmo_id = ?"
    cursor_lengkap.executemany(delete_placeholders, unique_keys)
    print(f"🗑️ Dihapus {cursor_lengkap.rowcount} baris lama dari {table_name_lengkap}")

# Bulk insert menggunakan to_sql (lebih cepat dari executemany)
df_to_insert_lengkap.to_sql(table_name_lengkap, conn_lengkap, if_exists="append", index=False, chunksize=1000)
conn_lengkap.commit()
conn_lengkap.close()
...
# ⚡ OPTIMASI: Faster SQLite mode
cursor_akhir.execute("PRAGMA synchronous = OFF")
cursor_akhir.execute("PRAGMA journal_mode = WAL")
...
# --- ⚡ OPTIMASI: Batch delete by unique keys (tidak per-row) ---
tanggal_batch_akhir = df_to_insert_akhir["tanggal"].unique().tolist()
if tanggal_batch_akhir:
    # Batch delete by tanggal (lebih cepat daripada per-row)
    delete_query = f"DELETE FROM {table_name_akhir} WHERE tanggal = ?"
    cursor_akhir.executemany(delete_query, [[tgl] for tgl in tanggal_batch_akhir])
    print(f"🗑️ Hapus {cursor_akhir.rowcount} baris lama dari {table_name_akhir}")

    # Bulk insert dengan chunksize
    df_to_insert_akhir.to_sql(table_name_akhir, conn_akhir, if_exists="append", index=False, chunksize=1000)
    conn_akhir.commit()
...
# ⚡ OPTIMASI: Faster SQLite mode
cursor_salah.execute("PRAGMA synchronous = OFF")
cursor_salah.execute("PRAGMA journal_mode = WAL")
...
# --- ⚡ OPTIMASI: Batch delete menggunakan executemany ---
tanggal_batch_salah = df_to_insert_salah["tanggal"].unique().tolist()

if tanggal_batch_salah:
    # Batch delete by tanggal
    delete_query = f"DELETE FROM {table_name_salah} WHERE tanggal = ?"
    cursor_salah.executemany(delete_query, [[tgl] for tgl in tanggal_batch_salah])
    print(f"🗑️ Dihapus {cursor_salah.rowcount} baris lama dari {table_name_salah}")

    # Bulk insert dengan chunksize
    df_to_insert_salah.to_sql(table_name_salah, conn_salah, if_exists="append", index=False, chunksize=1000)
    conn_salah.commit()
...
if len(data_error) > 0:
    ...
    if all(c in data_error.columns for c in key_cols):
        ...
        if unique_keys_to_process:
            # 2. Batch delete menggunakan executemany (lebih cepat)
            ...
            try:
                ...
                print(f"🔄 Dihapus {cursor.rowcount} baris lama untuk di-replace...")
                conn.commit()
        ...
        print(f"⚠️ Kolom kunci {key_cols} tidak ditemukan. Melewatkan batch delete.")

    # 3. Bulk insert dengan chunksize
    try:
        data_error.to_sql(table_name, conn, index=False, if_exists="append", chunksize=1000)
        conn.commit()
        conn.close()
