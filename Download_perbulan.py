...
for hari in range(1, num_days + 1):
    ...
    try:
        ...
        mapping_df = pd.read_excel('C:/Users/Lenovo/Downloads/streamlit/ww.xlsx')
        ...
        file_lokasi = 'C:/Users/Lenovo/Downloads/project/Stasiun.xlsx'  # Ganti ke path file lokasi (lon, lat)
...
# ==============================================================================
        ...
        # if not data_salah.empty:
        #     # --- [FIX 5] Ambil data full jam HANYA untuk stasiun & tanggal yang salah ---
        #     keys_salah = data_salah[["station_wmo_id", "tanggal_observasi"]].drop_duplicates()
        ...
        #     # --- Tambahkan kolom tanggal & jam untuk penyimpanan ---
        #     data_salah2["timestamp_data"] = pd.to_datetime(data_salah2["timestamp_data"])
            
        #     # !!! FIX DI SINI !!!
        #     # 'tanggal' HARUS SAMA DENGAN 'tanggal_observasi' agar dashboard bisa filter
        #     # JANGAN GUNAKAN: data_salah2["timestamp_data"].dt.date
        #     data_salah2["tanggal"] = data_salah2["tanggal_observasi"].astype(str)
        ...
        # else:
        #     # Buat DataFrame kosong jika tidak ada data salah
        #     data_salah2 = pd.DataFrame(columns=df_final.columns)
        #     data_salah2["tanggal"] = None
        #     data_salah2["jam"] = None
        #     data_salah2["tanggal_observasi"] = None # Tambahkan ini agar kolom konsisten
        ...
        # ]
        # cols_to_insert_salah = [col for col in cols_salah if col in data_salah2.columns]
        # df_to_insert_salah = data_salah2[cols_to_insert_salah].dropna(subset=['tanggal']) # Hindari insert baris kosong
        ...
        # if tanggal_batch_salah:
        #     for tgl in tanggal_batch_salah:
        #         if tgl is None: continue # Lewati jika ada None
        #         cursor_salah.execute(f"DELETE FROM {table_name_salah} WHERE tanggal = ?", (tgl,))
        #         print(f"🗑️ Hapus data lama tanggal {tgl} dari {table_name_salah}")
        ...
        # ]
        # cols_to_insert_salah = [col for col in cols_salah if col in data_salah2.columns]
        # df_to_insert_salah = data_salah2[cols_to_insert_salah]
        ...
        # kolom_pilihan = ["timestamp_data", "station_wmo_id",'NAME', "sandi_gts",'LAT',"LON",'ELEV',"Arah_angin","Kecepatan_angin","Gale","Temperatur","Tmin", "Tmax", "Tekanan_Permukaan","Curah_Hujan","Heavy_Rain"]
        # data_00 = df_jam00[kolom_pilihan]
        # df_final["tanggal_observasi"] = df_final["timestamp_data"].dt.date
        # mask_jam00 = df_final["timestamp_data"].dt.hour == 0
        # df_final.loc[mask_jam00, "tanggal_observasi"] = (df_final.loc[mask_jam00, "tanggal_observasi"] - pd.Timedelta(days=1))
        ...
        # data_merge["Curah_Hujan_Jam"] = data_merge["Curah_Hujan_Jam"].fillna(0)
        # data_merge["selisih"] = (data_merge["Curah_Hujan_Jam"] - data_merge["Curah_Hujan"]).abs()
        # data_akhir = data_merge[(data_merge["selisih"] <= 2) | ((data_merge["Curah_Hujan_Jam"] == 0) & (data_merge["Curah_Hujan"] == 0))].copy()
        # data_salah = data_merge.drop(data_akhir.index).copy()
        # #data_akhir["timestamp_data"] = data_akhir["timestamp_data"] - pd.Timedelta(days=1)
        ...
        #     cursor_akhir.execute(f"DELETE FROM {table_name_akhir} WHERE tanggal = ? AND jam = ? AND station_wmo_id = ?", (row["tanggal"], row["jam"], row["station_wmo_id"]))
        # df_to_insert_akhir = data_akhir[["tanggal","jam","station_wmo_id",'NAME','LAT',"LON",'ELEV',"sandi_gts","Curah_Hujan","Heavy_Rain","Curah_Hujan_Jam","Gale","Kecepatan_angin","Arah_angin","Temperatur","Tekanan_Permukaan","Tmin", "Tmax"]]
        # df_to_insert_akhir.to_sql(table_name_akhir, conn_akhir, if_exists="append", index=False)
        # conn_akhir.commit()
        # conn_akhir.close()
        # tanggal_batch_akhir = data_akhir["tanggal"].unique().tolist()
        # print(f"✅ Data AKHIR untuk tanggal {tanggal_batch_akhir} berhasil diupdate ke {db_path_akhir}")

        # # --- Proses dan Simpan ke data_salah.db ---
        # station_ids_salah = data_salah["station_wmo_id"].unique()
        # data_salah2 = df_final[df_final["station_wmo_id"].isin(station_ids_salah)].copy()
        # data_salah2["timestamp_data"] = pd.to_datetime(data_salah2["timestamp_data"])
        # data_salah2["tanggal"] = data_salah2["timestamp_data"].dt.date.astype(str)
        # data_salah2["jam"] = data_salah2["timestamp_data"].dt.strftime("%H:%M")
        # db_path_salah = "data_salah2.db"
        ...
        #     cursor_salah.execute(f"DELETE FROM {table_name_salah} WHERE tanggal = ? AND jam = ? AND station_wmo_id = ?", (row["tanggal"], row["jam"], row["station_wmo_id"]))
        # df_to_insert_salah = data_salah2[["tanggal","jam","station_wmo_id",'NAME','LAT',"LON",'ELEV',"sandi_gts","Curah_Hujan","Heavy_Rain","Curah_Hujan_Jam","Gale","Kecepatan_angin","Arah_angin","Temperatur","Tekanan_Permukaan","Tmin","Tmax",'tanggal_observasi']]
        # df_to_insert_salah.to_sql(table_name_salah, conn_salah, if_exists="append", index=False)
        # conn_salah.commit()
        # conn_salah.close()
        # tanggal_batch_salah = data_salah2["tanggal"].unique().tolist()
        # print(f"✅ Data SALAH untuk tanggal {tanggal_batch_salah} berhasil diupdate ke {db_path_salah}")

        # # --- Buat tabel otomatis dengan to_sql ---
        # # Kalau tabel belum ada, ini akan membuat tabel dengan semua kolom
        # # Jika sudah ada dan kamu ingin menambah data baru, gunakan if_exists='append'
        # # Tapi untuk update batch seperti punyamu, kita hapus dulu data duplikatnya
        # # Pastikan tabel sudah ada, kalau belum buat tabel kosong lewat to_sql dulu
        # if not conn.execute(f"SELECT name FROM sqlite_master WHERE type='table' AND name='{table_name}'").fetchone():
        #     print(f"Tabel {table_name} belum ada, membuat tabel baru...")
        #     df_final.head(0).to_sql(table_name, conn, if_exists="replace", index=False)
        # for _, row in df_final.iterrows():
        #     cursor.execute(f"""
        #         DELETE FROM {table_name}
        #         WHERE tanggal = ? AND jam = ? AND station_wmo_id = ?
        #     """, (row["tanggal"], row["jam"], row["station_wmo_id"]))

        # # --- Insert semua kolom ke tabel ---
        ...
        # tanggal_batch = df_final["tanggal"].unique().tolist()
        # print(f"✅ Semua kolom DataFrame berhasil diupdate ke {db_path} untuk tanggal {tanggal_batch}")
