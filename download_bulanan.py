...
for hari in range(1, num_days + 1):
    ...
    try:
        ...
        # konversi ke string agar tidak error
        ...
        # # --- Tentukan nama database dan tabel ---
        # db_path = "data_lengkap.db"
        # table_name = "data_lengkap"
        ...
        # db_path_kosong = "data_nddff.db"
        # table_name_kosong = "data_nddff"

        # conn_kosong = sqlite3.connect(db_path_kosong)
        # cursor_kosong = conn_kosong.cursor()

        # # --- Buat tabel baru (jika belum ada) ---
        # cursor_kosong.execute(f"""
        # CREATE TABLE IF NOT EXISTS {table_name_kosong} (
        #     tanggal TEXT,
        #     jam TEXT,
        #     station_wmo_id TEXT,
        #     NAME TEXT,
        ...
        # # --- Filter hanya data yang nddff-nya kosong atau NaN ---
        # df_kosong = df_final[df_final["nddff"].isna() | (df_final["nddff"].astype(str).str.strip() == '')]

        # # --- Hapus data lama dengan tanggal dan station yang sama ---
        # for _, row in df_kosong.iterrows():
        #     cursor_kosong.execute(f"""
        #         DELETE FROM {table_name_kosong}
        #         WHERE tanggal = ? AND jam = ? AND station_wmo_id = ?
        #     """, (row["tanggal"], row["jam"], row["station_wmo_id"]))

        # # --- Simpan data hasil filter ke database ---
        # if not df_kosong.empty:
        #     df_to_insert = df_kosong[[
        #         "tanggal", "jam", "station_wmo_id", "NAME", "LAT", "LON", "ELEV",
        #         "sandi_gts", "nddff", "Gale", "Kecepatan_angin", "Arah_angin"
        #     ]]
        #     df_to_insert.to_sql(table_name_kosong, conn_kosong, if_exists="append", index=False)
        #     conn_kosong.commit()
        #     tanggal_batch = df_kosong["tanggal"].unique().tolist()
        #     print(f"✅ Data dengan nddff kosong untuk tanggal {tanggal_batch} berhasil disimpan ke {db_path_kosong}")
        # else:
        #     print("ℹ️ Tidak ada data dengan nddff kosong ditemukan.")

        # # --- Tutup koneksi ---
        # conn_kosong.close()

        conn = sqlite3.connect("data_lengkap2.db")
        cursor = conn.cursor()

        # Loop tiap baris dan update hanya kolom tertentu
        for _, row in df_final.iterrows():
            cursor.execute(f"""
                UPDATE data_lengkap
                SET 
        ...
        conn.commit()
        conn.close()
        print("✅ Kolom nddff, Gale, Kecepatan_angin, dan Arah_angin berhasil diperbarui.")
